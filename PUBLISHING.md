...
- **Opening a room never touches your own notebook.** While you're in a room,
  what's on screen belongs to the room and is saved under the room's own key.
  Leave the room (or open the site without the `#room=` part) and your private
  notebook is exactly as you left it.
- **Pictures are shared too**, but shrunk on the way. A Firestore document
  holds a megabyte at most, so anything larger is scaled and re-encoded to fit
  before it goes into the room. Your own copy stays untouched at full quality;
  it's the shared copy that's smaller.
