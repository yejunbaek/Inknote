  ...
  window.api = {
    async load() {
      try {
        ...
        // A room's contents arrive over the network and replace whatever we
        // load here, so always start from this browser's own notebook.
    ...
    async save(data) {
      try {
        // While you're in a shared room, what's on screen belongs to the room,
        // not to you. Writing it to the usual key would quietly overwrite the
        // private notebook you had before you opened someone's link.
        const room = window.INKNOTE_SYNC && window.INKNOTE_SYNC.inRoom
          ? window.INKNOTE_SYNC.inRoom() : null;
        await idbPut(STORE_KV, room ? `room:${room}` : 'notebook', JSON.stringify(data));
        return { ok: true };
      } catch (err) {
    ...
    // Does this browser actually hold the bytes for this picture? A viewer
    // who joined a room has the reference but not the file.
    hasImage(src) {
      const name = imgName(src);
      return !!(name && objectUrls.has(name));
    },

    // Used by the room sync layer to upload a picture the room hasn't seen.
    async imageDataUrl(src) {
      const name = imgName(src);
      if (!name) return null;
      try {
        const blob = await idbGet(STORE_IMG, name);
        if (!blob) return null;
    ...
    // A picture that arrived from a room. Kept in IndexedDB like any other so
    // it survives a reload rather than being re-downloaded every time.
    async adoptImage(name, dataUrl) {
      if (!name || objectUrls.has(name)) return false;
      const blob = dataUrlToBlob(dataUrl);
      if (!blob) return false;
      objectUrls.set(name, URL.createObjectURL(blob));
      try { await idbPut(STORE_IMG, name, blob); } catch { /* cache only */ }
      return true;
    },
