...
(async () => {
  ...
  // ---- ink ---------------------------------------------------------------
  // Firestore rejects an array inside an array, and a stroke's points are
  // exactly that — so ink has to be flattened before it can be written.
  const ink = await page.evaluate(() => {
    const I = window.INKNOTE_SYNC.internals;
    const stroke = {
      id: 'st1', tool: 'pen', color: '#111', size: 3,
      points: [[0, 0, 0.5], [10, 4, 0.8], [20, 9, 1]]
    };
    const nb = {
      version: 2, variables: [], recentLinks: [],
      sections: [{ id: 's', name: 'S', color: '#000', pages: [
        { id: 'p1', title: 'One', items: [], strokes: [stroke], links: [] }
      ] }]
    };
    const docs = I.toDocs(nb);

    const nested = (v) => Array.isArray(v)
      ? v.some(x => Array.isArray(x) || nested(x))
      : (v && typeof v === 'object' ? Object.values(v).some(nested) : false);

    const back = I.fromDocs(docs.structure, docs.pages);

    // A room written by the old build still has to open.
    const legacy = I.fromDocs(docs.structure, {
      p1: { title: 'One', items: [], links: [], strokes: [stroke] }
    });

    // An echo of our own write must not look like a change.
    const last = {
      structure: JSON.stringify(docs.structure),
      pages: new Map(Object.entries(docs.pages).map(([k, v]) => [k, JSON.stringify(v)]))
    };
    const quiet = I.diffDocs(I.toDocs(back), last).changedPages.length === 0;

    // ...but a new stroke must.
    const drawnOn = JSON.parse(JSON.stringify(back));
    drawnOn.sections[0].pages[0].strokes.push({
      id: 'st2', tool: 'pen', color: '#111', size: 3, points: [[1, 1, 0.4]]
    });
    const noticed = I.diffDocs(I.toDocs(drawnOn), last).changedPages.map(c => c[0]);

    return {
      noNestedArrays: !nested(docs.pages),
      flat: JSON.stringify(docs.pages.p1.strokes[0].pts) === JSON.stringify([0, 0, 0.5, 10, 4, 0.8, 20, 9, 1]),
      roundTripped: JSON.stringify(back.sections[0].pages[0].strokes) === JSON.stringify([stroke]),
      legacyOk: JSON.stringify(legacy.sections[0].pages[0].strokes) === JSON.stringify([stroke]),
      quiet, noticed
    };
  });

  check(ink.noNestedArrays, 'a page with ink contains no nested arrays Firestore would reject');
  check(ink.flat, 'stroke points are flattened to x, y, pressure in order');
  check(ink.roundTripped, 'and rebuild into exactly the stroke that was drawn');
  check(ink.legacyOk, 'a room written by an older build still opens');
  check(ink.quiet, 'a page of ink coming back from the server is not written again');
  check(ink.noticed.length === 1 && ink.noticed[0] === 'p1', 'but a new stroke is queued for writing');

  // ---- picture sharing ---------------------------------------------------
  const pics = await page.evaluate(async () => {
    const I = window.INKNOTE_SYNC.internals;

    const nb = { sections: [{ pages: [
      { items: [
        { id: 'a', type: 'image', src: 'inknote-img://local/one.png' },
        { id: 'b', type: 'card', text: 'not a picture' },
        { id: 'c', type: 'image', src: 'inknote-img://local/two.png' },
        { id: 'd', type: 'image', src: 'inknote-img://local/one.png' },
        { id: 'e', type: 'image', src: null }
      ] }
    ] }] };

    // Random noise, because flat shapes compress away to nothing and
    // wouldn't exercise the shrinking path at all.
    const big = document.createElement('canvas');
    big.width = 1400; big.height = 1000;
    const ctx = big.getContext('2d');
    const data = ctx.createImageData(big.width, big.height);
    ...
    const huge = big.toDataURL('image/png');
    const shrunk = await I.shrinkDataUrl(huge, 700000);

    // A small one should be passed through untouched.
    const small = document.createElement('canvas');
    small.width = 4; small.height = 4;
    const smallUrl = small.toDataURL('image/png');
    const untouched = await I.shrinkDataUrl(smallUrl, 700000);

    // round-trip a picture through the adapter the way a room would
    const saved = await window.api.saveImage(smallUrl);
    const exported = await window.api.imageDataUrl(saved.url);
    const adoptedNew = await window.api.adoptImage('came-from-room.png', smallUrl);
    const adoptedAgain = await window.api.adoptImage('came-from-room.png', smallUrl);

    return {
      refs: I.imageRefs(nb),
      idFrom: I.imageIdFrom('inknote-img://local/abc.png'),
      idFromJunk: I.imageIdFrom('https://example.com/x.png'),
      hugeChars: huge.length,
      shrunkChars: shrunk ? shrunk.length : null,
      untouchedSame: untouched === smallUrl,
      exportedIsDataUrl: !!exported && exported.startsWith('data:'),
      hasOwn: window.api.hasImage(saved.url),
      hasStranger: window.api.hasImage('inknote-img://local/nope.png'),
      adoptedNew, adoptedAgain,
      adoptedResolves: window.api.imageUrl('inknote-img://local/came-from-room.png').startsWith('blob:')
    };
  });

  check(pics.refs.length === 2 && pics.refs.includes('one.png') && pics.refs.includes('two.png'),
    'every distinct picture in a notebook is found once');
  check(pics.idFrom === 'abc.png', 'a picture id is read out of its reference');
  check(pics.idFromJunk === null, 'and anything else is ignored');
  check(pics.hugeChars > 700000, 'the test picture really is too big for one document');
  check(pics.shrunkChars !== null && pics.shrunkChars <= 700000,
    'shrinking brings it under the document limit');
  check(pics.untouchedSame, 'a small picture is left exactly as it was');
  check(pics.exportedIsDataUrl, 'a stored picture can be read back out for upload');
  check(pics.hasOwn && !pics.hasStranger,
    'the app can tell which pictures this browser actually holds');
  check(pics.adoptedNew && !pics.adoptedAgain, 'a picture from a room is adopted once');
  check(pics.adoptedResolves, 'and then resolves like any other');
